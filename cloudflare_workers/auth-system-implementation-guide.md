...
### 4.1 How Activation Codes Tie Into Auth Flow

**Current system:**
- Users enter activation codes to unlock premium content
- Codes stored in `activation_codes` table
- Tracked in `trial_tracking` table
...
│  2. Account created with status: 'active'                   │
│  3. No premium access yet (activation_status: null)         │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│  - POST /activate with code + user_id                       │
│  - Validate code in activation_codes table                  │
│  - Update user record with activation details               │
│  - Record in trial_tracking                                 │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                   Premium Access Granted                     │
│  - User JWT now includes: { premium: true, expires: ... }   │
│  - App unlocks premium content                              │
└─────────────────────────────────────────────────────────────┘
```
...
async function activateCode(request, env) {
  ...
  const codeRecord = await getActivationCode(code, env);
  if (!codeRecord || codeRecord.used) {
    return errorResponse('Invalid or used activation code', 400);
  }

  // 2. Mark code as used
  await updateActivationCode(code, { used: 1, used_by: user_id }, env);

  // 3. Update user with premium access
  const expiresAt = calculateExpiry(codeRecord.duration_days);
  ...
  return jsonResponse({ success: true, premium_expires: expiresAt });
}
```

---

### 4.2 Stripe Subscription Sync with User Accounts
